...
Set_config_password(){
	...
	read -e -p "(默认: 123456):" ssr_password
...
Set_config_protocol(){
 ...
 ${Tip} 如果使用 auth_chain_a 协议，请加密方式选择 none，混淆随意(建议 plain)" && echo
	read -e -p "(默认: 5. auth_chain_a):" ssr_protocol
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/ymkf/shadowsocksr/archive/main.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b main https://github.com/main/shadowsocksr.git
	#[[ ! -e ${ssr_folder} ]] && echo -e "${Error} ShadowsocksR服务端 下载失败 !" && exit 1
	[[ ! -e "main.zip" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 下载失败 !" && rm -rf main.zip && exit 1
	unzip "main.zip"
	[[ ! -e "/usr/local/shadowsocksr-main/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf main.zip && exit 1
	mv "/usr/local/shadowsocksr-main/" "/usr/local/shadowsocksr/"
	[[ ! -e "/usr/local/shadowsocksr/" ]] && echo -e "${Error} ShadowsocksR服务端 重命名失败 !" && rm -rf main.zip && rm -rf "/usr/local/shadowsocksr-main/" && exit 1
	rm -rf main.zip
...
Service_SSR(){
	cp -f "/usr/local/shadowsocksr/ssr" "/etc/init.d/ssr"
	chmod +x /etc/init.d/ssr
	chkconfig --add ssr
	chkconfig ssr on
	echo -e "${Info} ShadowsocksR服务 管理脚本已安装 !"
}
# 安装 JQ解析器
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	cd /usr/local/shadowsocksr && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	cd /usr/local/shadowsocksr && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
