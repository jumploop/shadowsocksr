...
class TCPRelayHandler(object):
    ...
    def _create_encryptor(self, config):
        try:
            self._encryptor = encrypt.Encryptor(
                config['password'], config['method'], None, True
            )
            return True
        except Exception:
            ...
            try:
                ...
                while len(self._udp_data_send_buffer) > 6:
                    ...
                    if header_result is None:
                        continue
                    connecttype, addrtype, dest_addr, dest_port, header_length = (
                        header_result
                    )
                    if (addrtype & 7) == 3:
    ...
    def _get_redirect_host(self, client_address, ogn_data):
        ...
        if type(host_list) != list:
            host_list = [host_list]
        logging.info('host_list is %s', host_list)
    ...
    def _handle_stage_addr(self, ogn_data, data):
        try:
            ...
            if self._is_local:
                ...
                if header_result is None:
                    data = self._handel_protocol_error(self._client_address, ogn_data)
                    logging.info('_handel_protocol_error data is %s', data)
                    header_result = parse_header(data)
                self._overhead = self._obfs.get_overhead(
                    self._is_local
                ) + self._protocol.get_overhead(self._is_local)
                ...
                server_info.buffer_size = self._recv_buffer_size
            connecttype, addrtype, remote_addr, remote_port, header_length = (
                header_result
            )
            if connecttype != 0:
                pass
    ...
    def _check_auth_method(self, data):
        ...
        if socks_version != 5:
            logging.warning('unsupported SOCKS protocol version %s', str(socks_version))
            raise BadSocksHeader
...
class TCPRelay(object):
    ...
    def stat_add(self, local_addr, val):
        if self._stat_counter is not None:
            ...
            logging.debug(
                'port %d addr %s connections %d', self._listen_port, local_addr, newval
            )
